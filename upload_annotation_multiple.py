...
def get_task_ids(file):
    ...
    annotations = list(df['annotation_paths'])

    return annotations, task_ids
...
def dump_task(args): 
    ...
    for task_id in task_ids:
        subprocess.call(['python', 'cvat/utils/cli/cli.py', "--auth", auth_account, \
                            "dump", task_id, '--format', args.fileformat, args.save_root])

def upload_task(args):
    auth_account = f"{args.user_name}:{args.password}"
    
    annotations, task_ids = get_task_ids(args.tasks_upload)
    ...
    for i in range(len(task_ids)):
        ...
        subprocess.call(['python', 'cvat/utils/cli/cli.py', "--auth", auth_account, \
                            "upload", str(task_id), '--format', args.fileformat, annotation])
...
if __name__ == "__main__":
    ...
    ap.add_argument("--tasks_upload", type=str, help="csv file contains all videos u want to push", default = 'temp.csv')
    ap.add_argument("--fileformat", type=str, help="file format", default = "MOT 1.1")
    # ap.add_argument("--filename", type=str, help="file format", default = "test.zip")
    args = ap.parse_args()
    # create_task(args)
    upload_task(args)
